...
<head>
  ...
  <style>
	...
	.col-6 ul {list-style: none; margin: 0 0 5px 0; padding: 0;}
	.col-6 ul li {display: inline-block; width: 150px; text-align: left;}
...
<div class="container">
  ...
  <div class="row clearfix" style="float:left;">
    <div class="col-8 column">
      <hgroup class="headergroup">{{ $quotation->customer->name }}<br>
		<span>Date Needed: {{ Carbon::parse($quotation->date_needed)->format('F j, Y') }}</span><br>
		<span>RFQ No. {{ $quotation->customer_pr_no }}</span>   
	  </hgroup>
		...
		<p>
		  ...
		  @foreach ($quotation->customer->contactInfo->contactNumbers as $number)
		   {{$number->type_value}}: {{ $number->number }}<br />
		  @endforeach
		</p>
      ...
      <div style="width: 100%; display: inline-block;">
        <table style="width: 100%;">
          ...
          <tr class="frisco-data3">
            ...
            <td style="width: 50%; font-weight: bold; border-left: transparent; border-right: transparent;">P{{number_format($quotation->materials_price + $quotation->materials_tax, 2)}}</td>
          </tr>
          <tr class="frisco-data3">
            <td style="width: 50%; border-left: transparent; border-right: transparent;">Lot Price Discounts:</td><!--if there is no lot discount, can this not be shown?-->
            <td style="width: 50%; font-weight: bold; border-left: transparent; border-right: transparent;">P({{number_format($quotation->lot_discount, 2)}})</td>
          </tr>
          <tr class="frisco-data3">
            <td style="width: 50%; font-weight: bold; border-left: transparent; border-right: transparent;">Net Amount</td>
            <td style="width: 50%; font-weight: bold; border-left: transparent; border-right: transparent;">P{{number_format($quotation->materials_price_with_tax - $quotation->lot_discount, 2)}}</td>
          </tr>
        </table>
      </div>
    </div>
